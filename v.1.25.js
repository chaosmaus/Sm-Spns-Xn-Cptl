...
$(document).ready(() => {
    ...
    states =
        [
            ...
            {//1
                ...
                headerHeading: 'Financial Statements',
    ...
    function activateSpecialPage() {
        // Logic to activate the special page
        console.log('activate special page');
        $('.s-heading').text('Insurance Coverage');
        $('.s-description').text('For each coverage, enter the amount in annual premium ($)');
        $('.policies--block').removeClass('hidden');
    }

    function shouldActivateSpecialPage() {
        // Your condition for activating the special page
        return $('.policies--input--row').length > 1;
    }
    ...
    $('.s-button').on('click', (e) => {
        ...
        if (clickedButton.hasClass('next')) {
            ...
            if (state === 5) {
                ...
                if ($('#input-8-mask').hasClass('hidden') && isPolicy === false && $('.policies--input--row').length > 1) {

                    activateSpecialPage();
                    if (!($('.block--8').hasClass('hidden'))) $('.block--8').addClass('hidden');
                    state8 = true;
                    isPolicy = true;
                    //
                    return;
                } else {
                    if (!($('.policies--block').hasClass('hidden'))) $('.policies--block').addClass('hidden');
                }
            ...
            if (state === 6) {
                ...
                $('.s-info-panel--rich-text.licensing-regulation').hide()
                /* POLICIES */
                if ($('.policies--input--row').length > 1 && state8) {
                    state = 9;
